...
def load_linear_regression_data() -> tuple[pd.DataFrame, pd.Series]:
    """Use California Housing as a robust replacement for deprecated Boston loader."""
    dataset = fetch_california_housing(as_frame=True)
    return dataset.data, dataset.target


def load_polynomial_regression_data() -> tuple[pd.DataFrame, pd.Series]:
    """Load Auto MPG data, preferring seaborn and falling back to OpenML."""
    try:
...
def run(show_plot: bool = True) -> dict[str, float]:
    """Train linear and polynomial regression models and return their metrics."""
